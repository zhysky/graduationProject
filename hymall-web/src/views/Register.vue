          ...
          <v-col cols="12" class="d-flex justify-center">
            <v-card>
              ...
              <v-card-text>
                <ValidationObserver ref="observer">
                  <v-form ref="form" lazy-validation>
                    ...
                    <ValidationProvider
                      v-slot="{ errors}"
                      rules="required|min:4|confirmed:confirmation"
                    >
                      <v-text-field
                        label="确认密码"
                        v-model="password2"
              ...
              <v-card-actions>
                ...
                <v-spacer></v-spacer>
                <v-btn text small color="primary" to="/login">登录</v-btn>
                <v-spacer></v-spacer>
                <v-btn color="primary" text @click="submit">注册</v-btn>
              </v-card-actions>
            </v-card>
          </v-col>
        </v-row>
		...
		methods: {
			...
			submit () {
				this.$refs.observer.handleSubmit(async()=>{
					...
					if (!res.code) {
						const message={ username: [res.message] };
